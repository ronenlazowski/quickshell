...
Item {
    ...
    PopupWindow {
        ...
        MouseArea {
            ...
            hoverEnabled: true
            x: {
                if (!root.lastHoveredButton || !root.lastHoveredButton.window || !root.QsWindow) {
                    return (root.QsWindow?.window?.width ?? 0) / 2 - width / 2;
                }
                const mapped = root.QsWindow.mapFromItem(root.lastHoveredButton, root.lastHoveredButton.width / 2, 0);
                return mapped ? mapped.x - width / 2 : (root.QsWindow?.window?.width ?? 0) / 2 - width / 2;
            }
