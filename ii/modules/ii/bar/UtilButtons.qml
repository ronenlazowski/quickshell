...
Item {
    ...
    // Keep game mode state in sync with Hyprland settings
    Process {
        id: fetchGameModeState
        running: true
        command: ["bash", "-c", `test "$(hyprctl getoption animations:enabled -j | jq ".int")" -ne 0`]
        onExited: (exitCode, exitStatus) => {
            root.gameModeToggled = exitCode !== 0 // hyprctl returns 0 when animations enabled
        }
    }
    ...
    RowLayout {
        ...
        Loader {
            active: Config.options.bar.utilButtons.showGameModeToggle
            visible: Config.options.bar.utilButtons.showGameModeToggle
            sourceComponent: CircleUtilButton {
                Layout.alignment: Qt.AlignVCenter
                onClicked: {
                    root.gameModeToggled = !root.gameModeToggled
                    if (root.gameModeToggled) {
                ...
                MaterialSymbol {
                    ...
                    fill: 0
                    text: root.gameModeToggled ? "bolt" : "stadia_controller"
