    ...
    Loader {
        ...
        sourceComponent: Item {
            ...
            Image {
                ...
                // Avoid attempting to load internal qsimage handles from the C++ image provider
                // (these can be unknown when the provider doesn't expose the handle). If the
                // image is an internal qsimage URI, fall back to the app icon path instead
                // which avoids the QML warning and still shows a useful image.
                source: (root.image && (typeof root.image === "string" ? root.image : String(root.image)).indexOf("image://qsimage/") === 0) ?
                            Quickshell.iconPath(root.appIcon, "image-missing") : root.image
