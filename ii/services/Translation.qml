...
Singleton {
    ...
    component TranslationReader: FileView {
        ...
        // When true, create an empty translation file to avoid repeated FileNotFound warnings
        property bool ensureExists: false
        signal contentLoaded(var data)

        function reread() { // Proper reload in case the file was incorrect before
            const targetPath = `${translationReader.translationsDir}/${translationReader.languageCode}.json`;

            if (translationReader.ensureExists) {
                // Create the translations directory and a placeholder file to avoid noisy FileNotFound warnings
                Quickshell.execDetached(["bash", "-c", `mkdir -p '${translationReader.translationsDir}'`]);
                Quickshell.execDetached(["bash", "-c", `test -f '${targetPath}' || echo '{}' > '${targetPath}'`]);
            }

            translationReader.path = "";
            translationReader.path = targetPath;
